...
STAMP_TIMES = [0.0, 0.28, 0.44, 0.50, 0.68, 1.15]
STAMP_TRANSLATIONS = [
    (0.0, 1.65, 0.0),
    (0.0, 0.90, 0.0),
    (0.0, 0.22, 0.0),
...
def replace_channel(doc: GlbDocument, animation: dict, node_index: int, path: str, times, values, value_type: str) -> None:
...
def configure_top_down_camera(doc: GlbDocument) -> None:
    json_doc = doc.json
    try:
    ...
    camera_node["translation"] = [0.0, 5.8, 0.0]
...
def configure_stamp_animation(doc: GlbDocument) -> None:
    json_doc = doc.json
...
def main() -> None:
    ...
    doc.json.setdefault("asset", {})["generator"] = "tools/create_stamp_scene_glb.py top-down transform patcher"
